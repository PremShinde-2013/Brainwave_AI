                    ...
                    <ul className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                        {features.map((item, idx) => (
                            <GlareCard key={`l-${idx}`}>
                                ...
                                >
                                    <div className="w-fit transform-gpu rounded-full p-4 text-purple-600 border-white dark:[border:1px_solid_rgba(255,255,255,.1)] dark:[box-shadow:0_-20px_80px_-20px_#8686f01f_inset]">
                                        {item.icon}
                                    </div>
                                    <h4 className="text-lg font-bold tracking-tighter text-black dark:text-gray-300 font-geist">
                                        {item.title}
                                    </h4>
                                    <p className=" text-black dark:text-gray-500">{item.desc}</p>
                                </li>
                            </GlareCard>
                        ))}
                    </ul>
